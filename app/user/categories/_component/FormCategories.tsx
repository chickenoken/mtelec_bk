    ...
    <>
      <FormControl>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
          <Link href="/user/categories"><Button variant="outlined" color="secondary" startIcon={<FaArrowLeft />}>Back</Button></Link>
          {!id && <Button onClick={handleSave} variant="contained" className="bg-blue-600" startIcon={<FaRegSave />}>Save</Button>}
          {id && <Button onClick={handleUpdate} variant="contained" className="bg-blue-600" startIcon={<FaRegSave />}>Update</Button>}
        </Box>
        <Box className="w-1/2 mb-4">
          <TextField error={Boolean(errors.cate_name)} InputLabelProps={{ shrink: !!watch('cate_name') }} required fullWidth variant="outlined" size="small" label="Catergory Name" autoFocus {...register('cate_name')}/>
          {/* @ts-ignore */}
          {errors.cate_name && <Typography variant="caption" color={'red'}>{errors.cate_name.message}</Typography>}
        </Box>
        <Box className="w-40 mb-4">
          <Typography variant="h6" gutterBottom>Image Logo</Typography>
      ...
      {id && <>
          ...
          <Typography variant="h6" gutterBottom>Category Project Detail</Typography>
